...
`);
  ...
  switch (answer) {
    ...
    case "8":
      ...
      break;
    case "9":
      const twenties = students.filter(
    ...
    case "12":
      const average =
        students.reduce((sum, n) => sum + n.age, 0) / students.length;
      console.log(average);
      break;
    case "13":
      const onlyGirls = students.filter(
        (student) => student.gender === "female"
      );
      const avGirls =
        onlyGirls.reduce((sum, n) => sum + n.age, 0) / onlyGirls.length;
      console.log(avGirls);
      break;
    case "14":
      students.forEach((student) =>
        student.examScores.push(parseInt(Math.floor(Math.random() * 11)))
      );
      break;
    case "15":
